...
for session={'session1','session2','session3'}
     for subset={'G1','G2','G3','all'}
     mean_switchprob_winPN.(session{1}).(subset{1}).all=mean(asin(sqrt(switchprob_winPN.(session{1}).(subset{1}).all)),2);
     mean_switchprob_lossPN.(session{1}).(subset{1}).all=mean(asin(sqrt(switchprob_lossPN.(session{1}).(subset{1}).all)),2);
     ...
     mean_pct_winvsloss.(session{1}).(subset{1}).all=mean(asin(sqrt(pct_winvsloss.(session{1}).(subset{1}).all)),2);
    end
end

 plot_bargarph_3group_comparison_mean(mean_switchprob_winPN,'session1','reward positive bias','postive stay vs negative switch',[0,0.8],figdir)
 ...
 plot_bargarph_3group_comparison_mean(mean_switchprob_nowin,'session1','nowin','Switch probability',[0,0.6],figdir)
 plot_bargarph_3group_comparison_mean(mean_switchprob_loss,'session1','loss','Switch probability',[0,0.5],figdir)
 plot_bargarph_3group_comparison_pos_mean(mean_switchprob_win,'win','session1',[0.6,1],figdir)
 plot_bargarph_3group_comparison_pos_mean(mean_switchprob_noloss,'noloss','session1',[0.6,1],figdir)
 plot_bargarph_3group_comparison_mean(mean_pct_winvsloss,'session1','pct-win-vs-loss-chosen','win chosen vs loss chosen',[0,0.15],figdir)
 %%%stats
 %oneway anova
