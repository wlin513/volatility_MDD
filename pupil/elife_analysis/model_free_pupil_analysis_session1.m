...
clc;
%%
cd('D:/2017_clinical_study/code/pupil/elife_analysis');
getfolders;
addpath('D:/2017_clinical_study/code/plot_code/')
addpath('D:/2017_clinical_study/code/useful_fun/')
...
session1_sublist.G2(session1_sublist.G2==2034)=[];%exclueded from beh analysis
...
session1_sublist.G3(session1_sublist.G3==3025)=[];%somehow results for both vol block for the 1st session are nans
...
% excluded because 90% of trials chose one of the shapes on one of the
% blocks: sublist.all~=1010&sublist.all~=1016&sublist.all~=1017&sublist.all~=1021&sublist.all~=2032&sublist.all~=3034;

% session1_sublist.G1(session1_sublist.G1==1010)=[];
% session1_sublist.G1(session1_sublist.G1==1016)=[];
% session1_sublist.G1(session1_sublist.G1==1017)=[];
% session1_sublist.G1(session1_sublist.G1==1021)=[];
% session1_sublist.G2(session1_sublist.G2==2032)=[];
% session1_sublist.G3(session1_sublist.G3==3034)=[];
...
%%  
    ...
    for session={'session_1'}%,'session_2','session_3'}
       for sub=1:size(ssublist,2)    
                ...
                %% load pupil data 
                file_name=['out_',name,'_',session{1},'_normalized_within_session1_by_block.mat'];
                ...
                rew_delivered=rew_data(ifchosen_rew==1&blkindex&includetrials&rewnantrials&rew_include,:);
                rew_undelivered=rew_data(ifchosen_rew==0&blkindex&includetrials&rewnantrials&rew_include,:);
                pupil_delta_win_vol.(session{1}).all.all(sub,:)=mean(rew_delivered,1)-mean(rew_undelivered,1);
               ...
               %mean for loss vol blocks
                ...
                pun_delivered=pun_data(ifchosen_pun==1&blkindex&includetrials&punnantrials&pun_include,:);
                pun_undelivered=pun_data(ifchosen_pun==0&blkindex&includetrials&punnantrials&pun_include,:);
                pupil_delta_loss_vol.(session{1}).all.all(sub,:)=mean(pun_delivered,1)-mean(pun_undelivered,1);
             
        end
    
    pupil_delta_win.(session{1}).all.all=means_rew_delivered.(session{1}).all.all-means_rew_undelivered.(session{1}).all.all; %pupil response to rewards when chosen vs unchosen rewarded
    pupil_delta_loss.(session{1}).all.all=means_pun_delivered.(session{1}).all.all-means_pun_undelivered.(session{1}).all.all;
    pupil_delta_winvsloss.(session{1}).all.all=pupil_delta_win.(session{1}).all.all-pupil_delta_loss.(session{1}).all.all;
    adj_pupil_delta_win.(session{1}).all.all=squeeze(pupil_delta_win.(session{1}).all.all(:,2,:)-pupil_delta_win.(session{1}).all.all(:,3,:));
    adj_pupil_delta_loss.(session{1}).all.all=squeeze(pupil_delta_loss.(session{1}).all.all(:,3,:)-pupil_delta_loss.(session{1}).all.all(:,2,:));
    ...
    adj_pupil_delta_win1.(session{1}).all.all=squeeze(mean(pupil_delta_win.(session{1}).all.all(:,1:2,:),2)-pupil_delta_win.(session{1}).all.all(:,3,:));
    adj_pupil_delta_loss1.(session{1}).all.all=squeeze(mean(pupil_delta_loss.(session{1}).all.all(:,1:2:3,:),2)-pupil_delta_loss.(session{1}).all.all(:,2,:));
  
    end
    pupil_delta_win=get_full_structure(pupil_delta_win,trainingtype,session1_sublist);
    pupil_delta_loss=get_full_structure(pupil_delta_loss,trainingtype,session1_sublist);
    pupil_delta_winvsloss=get_full_structure(pupil_delta_winvsloss,trainingtype,session1_sublist);
    adj_pupil_delta_win=get_full_structure(adj_pupil_delta_win,trainingtype,session1_sublist);
    adj_pupil_delta_loss=get_full_structure(adj_pupil_delta_loss,trainingtype,session1_sublist);
    ...
    adj_pupil_delta_win1=get_full_structure(adj_pupil_delta_win1,trainingtype,session1_sublist);
    adj_pupil_delta_loss1=get_full_structure(adj_pupil_delta_loss1,trainingtype,session1_sublist);

%% plot figures
save('pupil.mat')
load('pupil.mat')
plot_pupil_for_each_block(pupil_delta_win,pupil_delta_loss,'session_1','received vs not',blkname,figdir)
plot_pupil_for_each_block(pupil_delta_winvsloss,pupil_delta_winvsloss,'session_1','win vs loss',blkname,figdir)
...
plot_pupil_for_mean(adj_pupil_delta_win1,adj_pupil_delta_loss1,'session_1','vol vs stable',figdir)
plot_pupil_for_mean(adj_pupil_delta_win,adj_pupil_delta_loss,'session_1','vol vs stable',figdir)
...
%%
data1=adj_pupil_delta_loss1.session_1.G1.all;
data2=adj_pupil_delta_loss1.session_1.G2.all;
data3=adj_pupil_delta_loss1.session_1.G3.all;
[orisigpoints, bootsigpoints, clustersigpoints] = cluster_based_mcc_ttest(data3(:,501:end)',0.05,1000);
[orisigpoints, bootsigpoints, clustersigpoints] = cluster_based_mcc_ttest2(data1,data3,0.05,1000);
%%
data=adj_pupil_delta_loss1.session_1.all.all;
[p,rbl,stats]=anova1(mean(data(:,1001:end),2),group);

for i=1:7
    [h,p]=ttest(mean(data3(:,1+i*500:(i+1)*500),2))
    mm=mean(adj_pupil_delta_loss.session_1.all.all(:,1+i*500:(i+1)*500),2);
    anova1(mm,group)
end 
