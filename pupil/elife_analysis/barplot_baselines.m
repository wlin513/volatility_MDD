...
ylabel('visit2 - visit1 pupil baseline')
xticklabels({'win','loss'})
%
saveas(f1,[figdir,'pupil/baseline_pupil_response_diff_average4blocks.png']);
%%    plot pupil response visit2 - visit1 for each block for the placebo group
f2=figure;
means=[mean(means_rew_bslines.placebo(2,:,1)-means_rew_bslines.placebo(1,:,1)),...
               ...
               mean(means_pun_bslines.placebo(2,:,1)-means_pun_bslines.placebo(1,:,1)),...
 ...
 stes=[std(means_rew_bslines.placebo(2,:,1)-means_rew_bslines.placebo(1,:,1))./sqrt(size(means_rew_bslines.placebo,2)),...
           ...
           std(means_pun_bslines.placebo(2,:,1)-means_pun_bslines.placebo(1,:,1))./sqrt(size(means_rew_bslines.placebo,2)),...
...
ylabel('visit2 - visit1 pupil baseline')
title('placebo')
xticklabels({'both vol','win vol','loss vol','both stable'})
saveas(f2,[figdir,'pupil/baseline_pupil_response_diff_for_each_block_placebo.png']);
%%    plot pupil response visit2 - visit1 for each block for the reboxetine group
f3=figure;
means=[mean(means_rew_bslines.reboxetine(2,:,1)-means_rew_bslines.reboxetine(1,:,1)),...
 ...
 stes=[std(means_rew_bslines.reboxetine(2,:,1)-means_rew_bslines.reboxetine(1,:,1))./sqrt(size(means_rew_bslines.reboxetine,2)),...
...
title('reboxetine')
xticklabels({'both vol','win vol','loss vol','both stable'})
saveas(f3,[figdir,'pupil/baseline_pupil_response_diff_for_each_block_reboxetine.png']);