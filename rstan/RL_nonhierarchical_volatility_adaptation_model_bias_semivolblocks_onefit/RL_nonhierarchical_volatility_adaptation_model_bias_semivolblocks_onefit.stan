...
parameters {
  ...
  real alpha_vol_win_tr[nsub]; // learning rate for win stable; one per participant shared between similiar schedules
  real alpha_vol_loss_tr[nsub]; // learning rate for loss stable; one per participant shared between similiar schedules
  ...
  real<lower=-1,upper=1> bias[nblk,nsub];
}
...
transformed parameters{
  ...
  for (is in 1:nsub){
    ...
    alpha[1,2,is] = Phi_approx(alpha_vol_win_tr[is]);
    alpha[1,1,is] = Phi_approx(alpha_vol_win_tr[is]+win_vol_adpt_tr[is]);
    
    alpha[2,1,is] = Phi_approx(alpha_vol_loss_tr[is]);
    alpha[2,2,is] = Phi_approx(alpha_vol_loss_tr[is]+loss_vol_adpt_tr[is]);
...
model {
  ...
  for (is in 1:nsub){
   alpha_vol_win_tr[is] ~ normal(0,2);
   alpha_vol_loss_tr[is] ~ normal(0,2);
   win_vol_adpt_tr[is] ~ normal(0,2);
   loss_vol_adpt_tr[is] ~ normal(0,2);
   beta_tr[is]  ~ normal(0,2);
   bias[1,is] ~ normal(0,2);
   bias[2,is] ~ normal(0,2);
  }
   ...
   for (iblk in 1:nblk){
    ...
    for (it in 2:ntr){
      if (includeTrial[it,iblk,is]==1){ // if there is no missing response
        ...
        opt1Chosen[it,iblk,is] ~ bernoulli_logit(WinEstProb1[it,iblk,is]-LossEstProb1[it,iblk,is]+bias[iblk,is]); // bernoulli is a distribution in the way as e.g. the 'normal distribution'
       }
      }
    }
  }
