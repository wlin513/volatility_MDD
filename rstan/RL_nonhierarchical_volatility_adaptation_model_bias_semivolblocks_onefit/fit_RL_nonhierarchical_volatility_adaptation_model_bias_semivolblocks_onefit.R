...
dest_dir<-"D:/2017_clinical_study/data/rstan/RL_nonhierarchical_volatility_adaptation_model_bias_semivolblocks_onefit"
stan_dir<-"D:/2017_clinical_study/code/rstan/RL_nonhierarchical_volatility_adaptation_model_bias_semivolblocks_onefit"
fig_dir<-"D:/2017_clinical_study/tmp_fig/rstan/RL_nonhierarchical_volatility_adaptation_model_bias_semivolblocks_onefit"
...
source(file.path(stan_dir,"read_ests_vol_adpt_model_bias_semivolblocks.R"))
...
choice=choice[,2:3,]
win_outcome=win_outcome[,2:3,]
loss_outcome=loss_outcome[,2:3,]
includeTrial=includeTrial[,2:3,]

#scan blocks
stan_data<-list(ntr=ntrials,nblk=2, nsub=nsubs, opt1Chosen=choice, opt1winout=win_outcome, opt1lossout=loss_outcome,includeTrial=includeTrial)
fit_scandata <- stan(file = file.path(stan_dir,'RL_nonhierarchical_volatility_adaptation_model_bias_semivolblocks_onefit.stan'), data = stan_data, iter = 10000, chains = 4, control=list(adapt_delta=0.8, max_treedepth = 10))
save(fit_scandata,file=file.path(dest_dir,"RL_nonhierarchical_volatility_adaptation_model_bias_semivolblocks_onefit.RData"))
#library("shinystan")
...
load(file=file.path(dest_dir,"RL_nonhierarchical_volatility_adaptation_model_bias_semivolblocks_onefit.RData"))

IDs=sub('\\/.*',"",subjects$V1)
blocknames=c("win volatile","loss volatile")
...
ggsave("alphas_RL_nonhierarchical_vol_adpt_model_bias_onefit.eps", plot = palpha, device = "eps", path = fig_dir,
...
library(R.matlab)
writeMat(file.path(dest_dir,'Result_vol_adpt_model_bias_semivolblocks.mat'),winalphas=winalphas,lossalphas=lossalphas,betas=betas,win_vol_adpt_tr=win_vol_adpt_tr,loss_vol_adpt_tr=loss_vol_adpt_tr)