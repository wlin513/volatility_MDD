...
parameters {
  ...
  real beta_tr[3,nsub];    // inverse temperature - ; one per participant 
...
transformed parameters{
  ...
  real<lower=0> beta[3,nsub];
  // transform the single-subject parameters
  for (is in 1:nsub){
    
    alpha[1,3,is] = Phi_approx(alpha_sta_win_tr[is]);
    alpha[1,2,is] = Phi_approx(alpha_sta_win_tr[is]+win_vol_adpt_tr[is]);
    alpha[1,1,is] = Phi_approx(alpha_sta_win_tr[is]+win_vol_adpt_tr[is]);
    
    alpha[2,3,is] = Phi_approx(alpha_sta_loss_tr[is]+loss_vol_adpt_tr[is]);
    alpha[2,2,is] = Phi_approx(alpha_sta_loss_tr[is]);
    alpha[2,1,is] = Phi_approx(alpha_sta_loss_tr[is]+loss_vol_adpt_tr[is]);
    for (iblk in 1:nblk){
